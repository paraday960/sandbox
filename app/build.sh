...
# بازگردانی خودکار SDK از فشرده‌ی داخل ورک‌اسپیس (اگر سندباکس ریست شده باشد)
...
"$BT/aapt2" link -o "$W/app.unsigned.apk" -I "$PLAT" \
  ...
  --version-code 5 --version-name 4.1
...
if [ ! -f "$ROOT/keys/debug.jks" ]; then
  mkdir -p "$ROOT/keys"
  "/usr/lib/jvm/jdk-11/bin/keytool" -genkeypair -keystore "$ROOT/keys/debug.jks" \
    -storepass android -keypass android -alias sandbox \
    -keyalg RSA -keysize 2048 -validity 10000 \
    -dname "CN=SandBox, O=SandBox, C=IR" 2>/dev/null
fi
"$BT/apksigner" sign --ks "$ROOT/keys/debug.jks" --ks-key-alias sandbox \
